...
### PO Review — 2026-07-08 (PO, father pass 50) — critique/polish pass. Fresh full sweep at HEAD `618e3b1` (last code commit `f972057`/178; board empty), re-verified the pass-47 X-6 BRA fix, and — this pass — drilled the training-HUD learned bar in a partly-filled state (23%) that prior dry passes only saw at 0%. **Found ONE buildable X-4/X-6 improvement:** the learned bar's unfilled track channel is invisible against its own backing panel, so the mastery meter doesn't read as a meter. No sign-off (Phase 10 owner-gated on its empty spec).

Fiftieth pass, fresh and stateless. HEAD `618e3b1`. The committed `build/web/` pck+wasm are timestamped 09:27 (post-178), so the bundle matches HEAD — served over http (`python3 -m http.server 8099`) and driven in headless Chromium at 390×844, deviceScaleFactor 2 (SwiftShader == the deployed GL Compatibility renderer), `env -u LD_LIBRARY_PATH`. The local Web build **bundles the licensed dog** (real cream Labrador / grey Nova / golden Sol / brown Balder rig). **Zero game console errors** on every run — training boot, `?bra_autotap=1` mark burst, completion menu, kennel grid, Nova modal.

**Pass-47 X-6 (BRA hero label) — still FIXED, re-verified in my own pixels.** 178's decoupled hero glyph (`T_DISPLAY` 74 + dedicated `font_display_black()` Baloo 2 ExtraBold wght 800, BRA-only) holds: in `P50-z-bra.png` the white «BRA» reads as a chunky, confident, extra-bold primary CTA. The shared wght-600 `font_display()` feeding the menu / coin readout / kennel titles / showcase stays byte-identical. Within visual parity of the goal art — no follow-up directive.

**Improvement (X-4 / X-6) — training HUD learned bar: the unfilled track channel is invisible, so the progress meter doesn't read as a meter.**
- *What I saw:* On the training HUD at **0 %** (the default for any fresh trick — every trick starts here) the bar shows only «Sitt … 0 %» text in a white pill with **no visible track** (`P50-z-hud.png`). At **23 %** (`P50-z-hud-try.png`, captured with `?bra_autotap=1&bra_force_tier=perfect`) the BLUE fill floats on the left but the **unfilled remainder to its right is invisible** — it blends into the opaque white backing panel — and there is **no knob/thumb** at the fill boundary.
- *Why it's wrong:* `learned_bar.gd` draws the track with `TRACK_COLOR = DesignSystem.PAPER` (line 47) **on top of** the `SCRIM_COLOR = DesignSystem.PAPER` backing panel (159, line 48) — track and panel are the **same white**, so the empty channel has zero contrast and the meter shape is only ever implied by the small blue nub. The goal art (`.docs/specs/assets/goal-training-screen.png`) shows a clearly-defined rounded **track channel** (a light groove) holding the blue fill, with a rounded **knob** at the fill edge, so "this meter fills as you learn" reads even when low or empty. In my build there is **no bar at all at 0 %**, undercutting the core Phase-2 mastery-progress feedback on the most-visible persistent surface.
- *What "good" looks like:* Keep 159's opaque panel + 145's dark INK labels / sun-bleed fix, but give the **unfilled track channel a tone visibly distinct from the backing panel** (a faint inset / slightly darker groove — e.g. `BORDER` or a light INK-tint) so the full rounded meter reads as a defined empty track at 0 % and the unfilled remainder stays visible when partly filled, matching the goal art. Optionally add the goal art's rounded **knob** at the fill boundary for the slider affordance. Verify at 0 %, ~25 %, ~60 %, and mastered (gold latch).

**Everything else swept clean — no other directive:**
- **Training page** (`P50-01-training.png`) matches the goal art otherwise: blue BRA hero CTA (178), «Triks»/«Kennel» blue nav pills (176, `HUD_NAV_INK = BLUE_INK` measured ~5.7:1 on the white pill — AA passes; the "pale" look is just the thin small glyph, not a contrast fail), gold coin pill, cream Labrador grounded facing camera on green grass, tan path → blue-roofed cottage, white picket fence, gold + rose coins.
- **Completion menu** (`P50-02-menu.png`): clean DS paper card — «Triks» heading + coin pill; Sitt («Trener nå», active pale-blue wash, dark ink), Ligg/Legg deg («Tilgjengelig»), Gi labb («Låst»); «Raser» Bella (Aktiv) / Brun lab («Adopter 30»); «Vis frem hundene» + «Gi tilbakemelding» secondaries; blue «Fortsett treningen» primary. Word/difficulty sections correctly progressive-disclosed (empty on a fresh dog — verified `word_rows`/`difficulty_rows` `[]`).
- **Kennel grid** (`P50-03-kennel-grid.png`): all **8 cells front-¾ face-to-viewer** (177 holds), plausible distinct coats, correct names/breeds/prices; badges are correct **Norwegian** dark-ink-on-accent labels — «Din hund» (green) · «Episk» (violet) · «Sjelden» (blue) · «Vanlig» (slate) · «Påskeegg» (rose). AA holds.
- **Kennel modal** (`P50-04-kennel-modal.png`, Nova): well-composed, AA-safe — «Episk» badge, front-¾ hero bust, dark «Nova» nameplate, warm blurb, 4 blue stat-pip rows (Læreevne/Energi/Mot/Fokus), «Lærevillig·Energisk·Intens» chips, «Unikt trekk / Øyet», «Kan lære: Sitt·Ligg·Legg deg», blue «Adopter 900» CTA.

**No regression** in any signed-off phase: closing the kennel restores the Phase-6 training page + Phase-1/2 core-loop scene; the coin economy / breeds / marker-word rows render in the completion menu as before. The «PERFECT» gold mark verdict + blue fill still land on a mark (`P50-z-hud-try.png`).

**No sign-off.** Phases 1/2/3/5/6/8/9 remain signed with no regression. Phase 10 (`phase10.md`) is still **empty/deferred** — owner-gated (no spec ⇒ cannot Visual-Review, cannot invent scope), so it can neither be signed off nor given directives; the learned-bar item above is a cross-cutting Phase-6-surface polish directive that keeps the loop improving while Phase 10 waits on the owner. The standing asset flags (distinct per-breed **models**, camera-facing **signature clips**, warm human "Bra!"/Maren voice, coat UV re-export) remain owner-gated.
