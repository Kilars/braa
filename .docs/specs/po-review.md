...
### PO Review — 2026-07-08 (PO, father pass 61) — critique/polish pass. Pass-60's completion-menu-subheading AA directive (186) **re-verified in pixels and holds** (pruned). **ONE new buildable X-6 directive:** the kennel **inspect-modal hero portrait renders each dog's coat ~2× brighter and hue-flipped vs its grid cell** — Nova is a dark charcoal border-collie in the grid (≈(45,48,52), cool) but a mid warm dove-grey in her modal bust (≈(91,88,84), warm), so the same dog reads lighter/warmer when you tap in to inspect it. Sweep at HEAD `a4d5802` (last code commit `a4d5802`/186; board empty). No sign-off (Phase 10 owner-gated on its empty spec).

Sixty-first pass, fresh and stateless. HEAD `a4d5802`; `build/web/` rebuilt at HEAD via `verify.sh` (gate green — Sitt-capable licensed Labrador bundled locally), served over http (`python3 -m http.server 8099`) and driven in headless Chromium at 390×844 (SwiftShader == the deployed GL Compatibility renderer), `env -u LD_LIBRARY_PATH`. **Zero game console errors** on every run — training boot, an `?bra_autotap=1` mark burst, the completion menu, the kennel grid + Nova modal.

**Improvement (X-6, cross-surface consistency) — kennel modal hero coat doesn't match the grid-cell coat.**
- **What I saw:** tapping **Nova** (the border collie) in the kennel grid opens her inspect modal, and her hero bust reads a **distinctly lighter, warmer grey** than the near-black charcoal she is in the grid thumbnail. Measured on the same running frame (`.screenshots/P61-kennel-modal.png`, crops `P61-nova-grid.png` / `P61-nova-modal.png`): grid-cell flank ≈ **(45,48,52)** — dark charcoal, cool (b>r); modal-hero flank ≈ **(91,88,84)** — mid dove-grey, warm (r>b). Roughly **2× the luminance and a cool→warm hue flip** for the identical dog. It's most glaring on Nova (the darkest cool coat); lighter coats (cream Bella, tan Sniff) swing less but the same brightening applies.
- **Why it's wrong:** the grid thumbnail is the promise; the modal is the payoff of tapping in to inspect — they should read as **one dog**. A charcoal border collie that becomes a mid-grey in its own hero shot loses the darkness that identifies the breed and breaks the "same dog" read across the two kennel surfaces (the exact cross-view coat-consistency the Phase-8 sign-off and task 117/147 care about; this slipped in when the modal moved to the live SubViewport). Root cause is **not** the tint: grid and modal share `_build_one_portrait` (same key 1.2 / fill 0.35 / ambient 0.9 light rig, same `NEUTRAL_COAT`, same `_band_dog_tint(portrait_tint)` — `kennel_screen.gd:598` grid, `:1177` modal). The **only** difference is yaw — the grid uses the variety `PORTRAIT_YAW_SPREAD[i]`, the modal uses dead-front `MODAL_PORTRAIT_YAW = 0.0` (`:131`) — so at the modal's front-on angle Nova's flank faces straight into the −34°/−38° key light and blows brighter, while the grid's steeper side-yaw keeps that flank in shadow. The coat value swings with pose angle instead of holding constant.
- **What "good" looks like:** the same dog reads the **same coat value + hue** in the grid cell and the modal hero — Nova stays a dark charcoal border collie in both. Balance the portrait light rig so apparent coat midtone doesn't swing so hard with yaw (e.g. soften the key-to-fill ratio / lift ambient / add a gentle rim from the opposite side so the front-lit flank doesn't wash), or otherwise match the modal's effective exposure to the grid's, applied in the shared `_build_one_portrait` so all 8 dogs stay consistent across both views. Keep the modal's fixed front-¾ `MODAL_PORTRAIT_YAW` hero framing (140) — only the brightness/hue swing needs fixing, not the angle.

**Pass-60 directive RESOLVED (completion-menu section subheadings «Raser»/«Merkeord»/«Vanskelighet» failed WCAG AA at SLATE_SOFT 2.87:1) — re-verified in pixels, holds, pruned.** Task 186 (`a4d5802`) repointed `BREED_SUBHEAD` / `WORD_SUBHEAD` / `DIFF_SUBHEAD` from `SLATE_SOFT` to `DesignSystem.SLATE` (`trick_menu.gd:243/256/260`). In my pixels — the «Raser» subheading over the breed rows (`.screenshots/P61-menu.png`, 4× zoom `P61-raser-zoom.png`) — the label ink now measures as SLATE-family (darkest glyph core ≈(115,129,144), clearly bluer/darker than SLATE_SOFT's (138,151,164)); the specified token is 5.28:1 on PAPER, AA-clean, and matches the kennel section-heading treatment (156). The locked-row `SLATE_SOFT` disabled labels correctly stayed as-is. (Merkeord/Vanskelighet share the identical const change and only render after their `MenuReveal` gates — first alt-word unlock / mastery understood — so they were dormant on this fresh boot; the fix is verified through «Raser».) Fixed and stable.

**Surface sweep — otherwise clean (re-verified this pass):**
- **Training page** (`P61-a-training.png`, apex `P61-mark-2.png`): matches the goal art — blue BRA hero CTA, hamburger + «Triks» + «Kennel» nav pills (full «Kennel», no truncation, 185 holds), gold coin pill «5000», «Sitt» learned bar with opaque track + `%`, cream Labrador (rear during a wander → faces camera + cyan approach ring at the seated apex), tan path → blue-roofed cottage, white picket fence, gold + rose coins.
- **Completion menu** (`P61-menu.png`): DS paper card — Sitt («Trener nå», active pale-blue wash, dark ink), Ligg/Legg deg («Tilgjengelig», SLATE names), Gi labb («Låst»); «Raser» Bella·Labrador («Aktiv») / Brun lab («Adopter 30» + coin); ghost secondaries + solid-blue «Fortsett treningen» primary; BRA correctly dimmed under the scrim. Section subheadings now AA-clean (186).
- **Kennel** (`P61-kennel-grid.png`, badge zooms `P61-bdg-nova.png` «Episk» / `P61-bdg-balder.png» «Sjelden»): 8 cells fill the portrait, front-¾ full-body poses, **Norwegian** rarity/status corner badges legible with dark ink on calm accents, coin price chips (owned Bella cell has none), plausible per-dog coats; the Nova modal opens on tap with a hero bust on a calm nameplate band, blurb, 4 blue stat meters, «Raseegenskaper» chips, «Unikt trekk / Øyet» card, «Kan lære: Sitt · Ligg · Legg deg», blue «Adopter 🪙 900» CTA. (Only the modal hero *coat brightness* falls short — see directive above.)

**No regression** in any signed-off phase: every surface renders as before, and closing the kennel/menu restores the Phase-6 training page + Phase-1/2 core mark loop. The directive above is a latent lighting-angle artifact in the shared kennel portrait rig, most visible on the darkest coat — not something 186 broke.

**No sign-off.** Phases 1/2/3/5/6/8/9 remain signed with no regression. Phase 10 (`phase10.md`) is still **empty/deferred** — owner-gated (no spec ⇒ cannot Visual-Review, cannot invent scope). This pass re-verifies the 186 subheading-AA fix, prunes it, and files **one measurement-backed X-6 directive** (kennel modal hero coat brighter/warmer than the grid cell). The standing asset flags (distinct per-breed **models**, camera-facing **signature clips**, warm human "Bra!"/Maren voice, coat UV re-export) remain owner-gated.
