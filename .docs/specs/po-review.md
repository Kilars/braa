...
### PO Review — 2026-07-08 (PO, father pass 49) — critique/polish pass. Fresh full sweep at HEAD `eb932fb` (178 shipped), re-verified the pass-47 X-6 BRA fix, and drilled deeper than prior passes on the breed-showcase CTA states. No buildable improvement found; no sign-off (Phase 10 owner-gated on its empty spec).

Forty-ninth pass, fresh and stateless. HEAD `eb932fb` (last code commit `f972057`/178; board empty). The committed `build/web/` pck+wasm are timestamped 08:57 (post-178), so the bundle matches HEAD — served over http (`python3 -m http.server 8099`) and driven in headless Chromium at 390×844, deviceScaleFactor 2 (SwiftShader == the deployed GL Compatibility renderer), `env -u LD_LIBRARY_PATH`. The local Web build **bundles the licensed dog** (real cream Labrador / grey Nova / golden Sol / brown Balder rig). **Zero game console errors** on every run — training boot, `?bra_autotap=1` mark burst, completion menu, kennel grid, Nova modal, breed showcase.

**Pass-47 X-6 (BRA hero label) — still FIXED, re-verified in my own pixels (pruned).** 178's decoupled hero glyph (`T_DISPLAY` 74 + dedicated `font_display_black()` Baloo 2 ExtraBold wght 800, BRA-only) holds: in `P49-02-training.png` the white «BRA» reads as a chunky, confident, extra-bold primary CTA, no longer a timid caption. The shared wght-600 `font_display()` feeding the menu / coin readout / kennel titles / showcase stays byte-identical. Within visual parity of the goal art — no follow-up directive.

**Full sweep — all persistent surfaces polished, no new directive:**
- **Training page** (`P49-02-training.png`) matches the goal art (`.docs/specs/assets/goal-training-screen.png`): blue BRA hero CTA (178), «Triks»/«Kennel» blue nav pills (176), gold coin pill, «Sitt … %» learned bar on the opaque cream rail (145), cream Labrador grounded facing camera on green grass, tan path → blue-roofed cottage, white picket fence, gold + rose coins.
- **Completion menu** (`P49-03-menu.png`): clean DS paper card — «Triks» heading + coin pill; Sitt («Trener nå», active pale-blue wash, dark ink), Ligg/Legg deg («Tilgjengelig»), Gi labb («Låst»); «Raser» Bella (Aktiv) / Brun lab («Adopter 30»); «Vis frem hundene» + «Gi tilbakemelding» secondaries; blue «Fortsett treningen» primary. Word/difficulty sections correctly progressive-disclosed (empty on a fresh dog — verified `word_rows`/`difficulty_rows` `[]`). Compact card, no scroll clipping.
- **Kennel grid** (`P49-06-kennel-grid.png`): all **8 cells front-¾ face-to-viewer** (177 holds), plausible distinct coats, correct names/breeds/prices; zoomed the badges (`P49-z-badges-top.png`) and confirmed correct **Norwegian** dark-ink-on-accent labels — «Din hund» (green) · «Episk» (violet) · «Sjelden» (blue) · «Vanlig» (slate) · «Påskeegg» (rose). AA holds.
- **Kennel modal** (`P49-07-modal-*.png`, Nova): well-composed, AA-safe — «Episk» badge, front-¾ hero bust, dark «Nova» nameplate, warm blurb, 4 blue stat-pip rows (Læreevne/Energi/Mot/Fokus), «Lærevillig·Energisk·Intens» chips, «Unikt trekk / Øyet», «Kan lære: Sitt·Ligg·Legg deg», blue «Adopter 900» CTA.
- **Breed showcase** (`P49-08-showcase.png`, opened via «Vis frem hundene»): dark header «Mine hunder / Bella — aktiv / Labrador», live front-¾ dog in the garden, HUD hidden. Drilled the commit CTA + chevrons harder than prior passes, and this time **verified the 2-owned-dog case in my own pixels** (adopted «Brun lab» for 30 coins via the completion menu — balance 2000→1970, roster grew to `[labrador, chocolate_labrador]`, Phase-3 economy intact — then re-opened the showcase, `P49b-*`): (1) with 2 owned dogs the ◀▶ chevrons **appear** and genuinely cycle the spotlight (`showcase_spotlit` chocolate→labrador on «next», back on «prev»), and the live rig re-tints per spotlit breed (brown Brun lab ↔ cream Bella); (2) the **CTA hierarchy flips correctly** — «Tren denne» renders the **muted DISABLED fill when the spotlit dog IS the active one** (`P49b-02`, Brun lab active+spotlit) but a **saturated DS blue-gradient primary when a non-active dog is spotlit** (`P49b-03`, Bella spotlit while Brun lab active), matching `breed_showcase_view.gd:80-106` (`commit_disabled_fill()` vs the cached blue-gradient stylebox); (3) with **≤1 owned dog the chevrons are HIDDEN** (not merely inert — `chevrons_active(owned_count)` → false, task 164), with the caption switched to «Adopter flere hunder for å bla» so no dead control looks live. No false-primary / dead-control / hierarchy defect on any path.

**No regression** in any signed-off phase: closing the kennel restores the Phase-6 training page + Phase-1/2 core-loop scene; the coin economy / breeds / marker-word rows render in the completion menu as before.

**No sign-off, no new directive.** Phases 1/2/3/5/6/8/9 remain signed with no regression. Phase 10 (`phase10.md`) is still **empty/deferred** — owner-gated (no spec ⇒ cannot Visual-Review, cannot invent scope), so it can neither be signed off nor given directives. Hunting hard across every surface — including a deeper showcase-CTA drill this pass — turned up no further buildable quality gap against the vision, the goal art, or the design system. The standing asset flags (distinct per-breed **models**, camera-facing **signature clips**, warm human "Bra!"/Maren voice, coat UV re-export) remain owner-gated.
