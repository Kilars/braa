...
### PO Review — 2026-07-09 (PO, father pass 68) — critique/polish pass. Pass-67's section-heading directive (task 193) **re-verified in pixels and resolved** (pruned): «Raser», «Markørord» and «Vanskelighet» now all draw through one `_draw_subheading` at 18px Nunito body-bold SLATE with no divider — captured with all three sections revealed (2 masteries, `.screenshots/P68-menu-3sections.png`), they read as one system clearly subordinate to the 26px Baloo-2 «Triks» title. **ONE new buildable X-6/X-4 directive:** the «Vanskelighet» section is the only one of the four menu selection sections whose *selectable* alternate rows carry **no right-side action badge** — Hard/Ekspert on a normal dog show their trade hint but nothing signalling they're tappable, while the parallel «Markørord»/«Raser» switch rows all show a blue «Bytt». Sweep at HEAD `4740c31` (last code commit `4740c31`/193; board empty). No sign-off (Phase 10 owner-gated on its empty spec).

Fresh, stateless pass. HEAD `4740c31`; `build/web/` rebuilt at HEAD via `verify.sh` (gate green — Sitt-capable licensed Labrador bundled locally, ~39 MB pck refreshed 01:37), served over http (`python3 -m http.server 8099`) and driven in headless Chromium at 390×844 (SwiftShader == the deployed GL Compatibility renderer), `env -u LD_LIBRARY_PATH`. **Zero game console errors** on every run — training boot, two `?bra_autotap=1` mastery bursts (Sitt → 100% then a trick-switch to Ligg → 100%, +10 coins each, marker words + difficulty sections revealed), the completion menu (via the «Triks» pill), the kennel grid + Balder/Lykke/Sniff modals, the breed showcase.

**Pass-67 directive RESOLVED (193 unified the completion-menu section headings) — re-verified in pixels, pruned.** Task 193 (`4740c31`) routed «Raser», «Markørord» and «Vanskelighet» through ONE `_draw_subheading` helper drawing `subhead_font()` (= Nunito body-bold) at `SUBHEAD_SIZE` = 18px, demoted «Markørord» off its old 26px Baloo-2 display treatment, and dropped its unique divider (`WORD_HEADER_H` 38→30 to match the peers). I drove the menu to reveal **all three sections at once** (mastered Sitt, switched to Ligg, mastered Ligg → 2 masteries) and zoomed each heading (`.screenshots/P68-z-raser.png`, `P68-z-markord3.png`, `P68-z-diff.png`, `P68-z-triks.png`): the three subheadings now render at an identical small size/weight/font/SLATE colour with no dividers and a uniform section-break, clearly subordinate to the larger, heavier Baloo-2 «Triks» panel title. The card's typographic hierarchy reads as one system. Resolved; not re-filed.

**Improvement (X-6 / X-4, «Vanskelighet» rows don't signal selectability like the three peer sections) — the difficulty section is the only menu selection list whose tappable alternates carry no action badge.**
- **What I saw:** with the menu revealing all four selection sections (`.screenshots/P68-menu-3sections.png`, zooms `P68-z-diff.png` vs `P68-z-words2.png`), the switch rows are marked inconsistently. In **«Markørord»** every switchable alternate carries a right-aligned blue **«Bytt»** badge (Dyktig! «+15% · hviler 2» **«Bytt»**, Flink! «+20% · hviler 2» **«Bytt»**), and «Raser» does the same (Brun lab «Adopter 🪙 30»); the current item shows a dark-ink state word (Bra! «Aktiv», Bella «Aktiv», Normal «Valgt»). But in **«Vanskelighet»** only the *selected* row is badged (Normal «Valgt»); **Hard** and **Ekspert** — the two rows a normal dog CAN tap to switch mode (P4-1) — draw their trade hint under the name and then **nothing on the right at all** (`trick_menu.gd:_draw_difficulty_row`, badge stays `""` for non-active rows; the code comment literally reads *"other rows show no badge"*). Every other selection section (tricks «Tilgjengelig», breeds «Bytt», words «Bytt») marks its selectable rows with a right-side badge; difficulty is the lone exception.
- **Why it's wrong:** the four sections are structural peers — "one item is current, the rest are selectable." Passes 152/167/170 deliberately unified them so tricks/breeds/words/difficulty "read as one system." Leaving Hard/Ekspert with no right-side affordance breaks that: a player scanning the card sees «Bytt» telling them the *words* are switchable but gets no such signal on the *difficulty* alternates, so Hard/Ekspert read as static/informational rows rather than the tappable mode-switch they are — the switch is measurably less discoverable, and it's exactly the "same component rendered differently across the surface" the polish lens targets.
- **What "good" looks like:** give the selectable (non-active, non-locked) difficulty rows a right-aligned **«Bytt»** action badge in the SAME blue action-ink the word/breed switch rows use, so «Vanskelighet» marks selectability like its three peers. Keep Normal's dark-ink «Valgt» current-state badge, keep the special-dog locked «Låst» + the "why locked" note (119/122), keep the dimmed trade subtitle (121). AA-clean, right-aligned at the same 14px inset as the other badges. Net: all four menu sections signal "current vs switchable" identically.

**Surface sweep — otherwise clean (re-verified this pass, in my own pixels):**
- **Training page** (`P68-a-training.png`, `P68-topbar-zoom.png`): matches the goal art — blue BRA hero CTA, hamburger + «Triks» + «Kennel» nav pills (full «Kennel», no truncation; `BLUE_INK` labels AA), gold coin pill «5000», «Sitt … 0%» learned bar with opaque track, cream Labrador centred/grounded on grass (faces camera at apex, wanders rear-to-camera between per P2-8), tan path → blue-roofed cottage, white picket fence, gold + rose coins.
- **PERFECT payoff** (`P68-menu-mastered.png` catches it behind the scrim): the reward beat lands with punch — big gold «PERFECT» display text top-centre, dog snaps to a clean seated apex, learned bar ticks to 100%, +10 coins. Good juice.
- **Completion menu** (`P68-menu-3sections.png`, row zooms `P68-rows-trick.png` / `P68-rows-breedword.png`): DS paper card — trick rows (Sitt «Lært» after mastery / Ligg «Trener nå» active pale-blue / Legg deg «Tilgjengelig» / Gi labb «Låst»), «Raser» (Bella·Labrador «Aktiv» / Brun lab «Adopter 🪙 30»), «Markørord» (Bra! «Aktiv» / Dyktig!·Flink! «Bytt» / Super! «Låst»), «Vanskelighet» (Normal «Valgt» / Hard / Ekspert — the badge gap above). Progressive reveal (`MenuReveal`) staggers correctly (words at mastery #1, difficulty at #2). Clean button hierarchy — blue-ghost «Vis frem hundene» / «Gi tilbakemelding», solid-blue «Fortsett treningen» primary.
- **Breed showcase** (`P68-s-showcase.png`, footer `P68-s-footer2.png`): «Mine hunder» / «Bella — aktiv» / «Labrador» header, cream Labrador spotlit facing camera on the garden, page-dot «Bella» indicator, «Adopter flere hunder for å bla» hint, «Trener nå» (pale, non-tappable status; measured **5.95:1**) + «Tilbake» (dark pill, **8.11:1**) controls — both AA-clean (169/171 hold).
- **Kennel** (`P68-k-grid.png`, modals `P68-k-modal-balder.png` / `P68-k-modal-lykke.png`): 8 cells fill the 844 portrait, front-¾ full-body poses, Norwegian rarity/status corner badges (Bella «Din hund»/green, Nova «Episk»/violet, Balder+Sol «Sjelden»/blue, Pontus/Lykke/Sniff «Vanlig»/slate, Trulte «Påskeegg»/coral — all dark-ink AA per 149), gold coin price chips (owned Bella cell has none, keeps its faint-green ownership tint). Modals: dark hero bust matching the grid cell, blurb, 4 blue stat meters with the empty segment a present slate track (188), «Raseegenskaper» chips, «Unikt trekk», «Kan lære: Sitt · Ligg · Legg deg», solid-blue «Adopter 🪙 N» primary, unified light-disc ✕ (189). Trulte's coat stays the clean silver-white from 192.
- **Menu section headings** — the pass-67 fix (193): verified unified, see the resolved-directive note above.

**Considered but NOT filed — owner-gated / intentional, not buildable polish.** (1) The four warm-brown breeds (Balder/Schäferhund, Pontus/Gravhund, Lykke/Spisshund, Sniff/Beagle) read as similar mid-browns — but their catalog tints already span a genuine value/hue range and their real identities are **pattern/shape** (shepherd saddle, dachshund body, beagle tricolor) a single-colour tint on the one shared Labrador rig cannot convey; the honest fix is distinct per-breed **models** (BUST-068, P3-D1/D2/D4), a standing owner flag. (2) In «Raser», Bella draws a two-line name+breed («Bella / Labrador») while «Brun lab» draws one line — a known dual-roster artifact (173: kennel individuals carry a breed subtitle, legacy Phase-3 breeds don't); adding «Labrador» under «Brun lab» would be redundant (the name already says it's a Lab), so not filed.

**No regression** in any signed-off phase: every surface renders as before, two autotap bursts mastered Sitt then Ligg clean (bar → 100%, +10 coins each, PERFECT payoff + word-pop), the trick-switch worked, and closing the kennel/menu restores the Phase-6 training page + Phase-1/2 core mark loop (cream Labrador centred facing camera on green grass, tan path, cottage, fence, coins, blue BRA).

**No sign-off.** Phases 1/2/3/5/6/8/9 remain signed with no regression. Phase 10 (`phase10.md`) is still **empty/deferred** — owner-gated (no spec ⇒ cannot Visual-Review, cannot invent scope). This pass re-verifies the 193 section-heading fix, prunes it, and files **one buildable X-6/X-4 directive** (badge the selectable «Vanskelighet» rows with «Bytt» so all four menu selection sections signal current-vs-switchable identically). The standing asset flags (distinct per-breed **models**, camera-facing **signature clips**, warm human "Bra!"/Maren voice, coat UV re-export) remain owner-gated.
