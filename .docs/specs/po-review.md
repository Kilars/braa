...
### PO Review — 2026-07-09 (PO, father pass 74) — critique/polish pass. **ONE new buildable X-6 DS-consistency directive filed** (the owned-dog corner badge renders **two different words** across the kennel grid vs the inspect modal). Pass-73's re-verified items (198 «Trener nå» tint unify) remain settled and are not re-listed.

Fresh, stateless pass. HEAD `37ae959` (board empty); `build/web/` was already rebuilt at HEAD (~39 MB `index.pck`, 07:18 — the Sitt-capable licensed Labrador bundled locally), served over http (`python3 -m http.server 8099`) and driven in headless Chromium at 390×844 (SwiftShader == the deployed GL Compatibility renderer), `env -u LD_LIBRARY_PATH`. **Zero game console errors** on every run — training boot, a 40-tap BRA mark burst (dog engages, faces camera, cyan approach ring lands on the apex — `P74-b-after-marks.png`), the «Triks» menu, the breed showcase, the kennel grid + Sol/Bella modals (`tools/po_pass74*.mjs`).

**Bugfix / Change — X-6 (DS consistency): the owned-dog ownership corner badge renders «Din hund» in the kennel GRID but «Din» in the inspect MODAL — the identical green corner-badge component shows two different words for the same dog (Bella) across two surfaces.** Verified in my own pixels: grid cell badge reads **«Din hund»** (`.screenshots/P74b-grid-badge.png`, 4× clip), the modal hero badge reads **«Din»** (`.screenshots/P74b-modal-badge.png`, 4× clip) — same green pill, same top-left corner, same dog. Root cause is a real code divergence, not a render artefact: the grid draws the badge from `status_label` (`kennel_dog.gd:245` → «Din hund») while the modal deliberately forces the `rarity_label` path (`kennel_screen.gd:1319-1326` → `kennel_dog.gd:136` → «Din»). This is the **same class of defect the pass-72 fix closed** (the «Trener nå» pill rendering two tints across showcase↔kennel, unified in task 198) — a shared status component that drifted between surfaces — and it is **novel**: 148 introduced the rarity badge and 149 de-duped the redundant owned *price* pill, but no pass ever unified this badge *word* between grid and modal. It is the lone remaining cross-surface component that isn't one language. **Note the secret dog (Trulte) does NOT diverge** — «Påskeegg» renders on both grid and modal — and all buyables match («Vanlig»/«Sjelden»/«Epik» on both), so the fix is scoped to the single owned case. **What "good" looks like:** the owned green corner badge reads **one word on both surfaces**. Recommend unifying the modal to the grid's **«Din hund»** (the warmer, unambiguous ownership-status phrase — "your dog" — and it parallels the secret badge already showing the full «Påskeegg» on both surfaces); alternatively unify the grid down to «Din». Either way, source both from one label so they can't drift again (the point of the DS-unify lever, exactly as 198 sourced the showcase pill fill from the kennel's own `active_state_fill()`). Green ownership hue, dark-ink AA (149), corner position, and pill shape all stay; only the word aligns.

**Surface sweep — otherwise clean (re-verified this pass, in my own pixels):**
- **Training page** (`P74-a-training.png`): matches the goal art (`assets/goal-training-screen.png`) — blue BRA hero CTA with bold white label + 3D lip, hamburger + «Triks» + «Kennel» nav pills (full «Kennel», no truncation), gold coin pill, «Sitt … 0%» learned bar with opaque cream track, cream Labrador grounded on grass (wanders on idle, faces camera + cyan ring at a seated apex), tan path → blue-roofed cottage, white picket fence both sides, gold + rose coins, bushes. Core mark loop drives clean across a 40-tap burst.
- **«Triks» menu** (`P74-c-menu.png`): DS paper card — «Triks» Baloo-2 title over coin pill, trick rows (Sitt «Trener nå» / Ligg · Legg deg «Tilgjengelig» / Gi labb «Låst»), «Raser» (Bella «Aktiv» / Brun lab «Adopter 30»), blue-ghost «Vis frem hundene» + «Gi tilbakemelding» + solid-blue «Fortsett treningen» primary. Row names flush-aligned (195), badges right-aligned at a common inset. **Verified the missing «Markørord»/«Vanskelighet» sections are intentional progressive disclosure** (task 127/128: words reveal once an alt word unlocks, difficulty once a trick is mastered — a fresh session correctly hides both; `main.gd:2374-2375`), not a regression.
- **Breed showcase** (`P74-d-showcase.png`): «Mine hunder» / «Bella — aktiv» white titles + «Labrador» scrim-chip caption (196, AA-clear), cream Labrador spotlit on the garden (HUD hidden), «Bella» page-dot chip, «Adopter flere …» hint scrim chip (196), green-mint «Trener nå» (198, unified) + «Tilbake» dark pill bottom chrome — all AA-clear.
- **Kennel** (`P74k-grid.png` grid, `P74k-modal-sol.png` buyable modal, `P74k-modal-bella.png` owned modal): 8 cells fill the 844 portrait under «Kennelen · Profesjonell fasilitet · 8 plasser» + coin chip, front-¾ full-body poses, Norwegian rarity/status corner badges (Nova «Epik»/violet, Balder+Sol «Sjelden»/blue, Pontus/Lykke/Sniff «Vanlig»/slate, Trulte «Påskeegg»/coral — dark-ink AA per 149), gold coin price chips by rarity (owned Bella none + faint-green tint; Trulte «Gratis»). Sol modal: golden hero bust, «Sol» nameplate, blurb, 4 blue stat meters with the empty segment a present slate track (188), «Raseegenskaper» chips (Vennlig · Glad · Tillitsfull), «Unikt trekk» (Alles venn), «Kan lære: Sitt · Ligg · Legg deg», solid-blue «Adopter 500» primary CTA reading correctly as primary. Bella owned modal mirrors it with the green-mint «Trener nå» non-tappable pill. Coats: Bella cream, Sol golden, Trulte clean silver-white (192), Nova dark.

**Considered but NOT filed — owner-gated / intentional / re-litigation, not buildable polish.** (1) The four warm-brown breeds (Balder/Schäferhund, Pontus/Gravhund, Lykke/Spisshund, Sniff/Beagle) read as similar mid-browns and Nova (Border collie) reads as a uniform dark-grey, not the iconic black-and-white — their real identities are **pattern/shape** a single-colour tint on the one shared Labrador rig cannot convey; the honest fix is distinct per-breed **models** (BUST-068, P3-D1/D2/D4), a standing owner flag. (2) The active-marker vocabulary differs across surface families — the completion menu marks active selections in its **blue** active-wash language (trick «Trener nå», breed «Aktiv», difficulty «Valgt» — deliberately unified by the 152/167/pass-68 arc), while the kennel/showcase mark the active **dog** in the **green** ownership language. Each is internally coherent and deliberate; forcing one onto the other would break the settled arc it belongs to. (3) The trick-row «Tilgjengelig» vs the other sections' «Bytt» switch badge — kept (tricks carry a «Lært» learned state the others lack). (4) The showcase / training dog is a live idle that can turn rear/side between frames — animation timing, not a fixable layout defect.

**No regression** in any signed-off phase: every surface renders as before, the core mark loop + «Triks» menu + breed showcase + kennel grid + Sol/Bella modals all drive clean, closing back restores the Phase-6 training page + Phase-1/2 core mark loop. Zero console errors throughout.

**No sign-off.** Phases 1/2/3/5/6/8/9 remain signed with no regression. Phase 10 (`phase10.md`) is still **empty/deferred** — owner-gated (no spec ⇒ cannot Visual-Review, cannot invent scope). This pass files **one buildable X-6 directive** (unify the owned corner-badge word «Din hund» vs «Din» across grid↔modal), otherwise the polish/AA/DS arc holds and the remaining shortfalls are the standing owner-gated asset flags (distinct per-breed **models**, camera-facing **signature clips**, warm human "Bra!"/Maren voice, coat UV re-export).
