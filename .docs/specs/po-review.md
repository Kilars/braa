...
### PO Review — 2026-07-08 (PO, father pass 63) — critique/polish pass. Pass-62's kennel-modal stat-meter empty-segment directive (188) **re-verified in pixels and resolved** (pruned). **ONE new buildable X-6/X-4 directive:** the kennel **modal ✕ close control is an under-sized, low-contrast, off-standard affordance** — 36 u (below the app's own 44 u control size) with a white glyph on translucent black that washes out where it overlaps the light portrait area, and it's drawn *differently* from the grid's own back-✕ (dark glyph on light circle). Sweep at HEAD `c98f623` (last code commit `c98f623`/188; board empty). No sign-off (Phase 10 owner-gated on its empty spec).

Fresh, stateless pass. HEAD `c98f623`; `build/web/` rebuilt at HEAD via `verify.sh` (gate green — Sitt-capable licensed Labrador bundled locally), served over http (`python3 -m http.server 8099`) and driven in headless Chromium at 390×844 (SwiftShader == the deployed GL Compatibility renderer), `env -u LD_LIBRARY_PATH`. **Zero game console errors** on every run — training boot, the completion menu, the kennel grid + Nova/Sol/Bella modals, the breed showcase.

**Improvement (X-6/X-4, affordance clarity + cross-surface consistency) — the kennel modal ✕ close button is small, faint, and inconsistent with the grid's own back-✕.**
- **What I saw:** the inspect-modal close control (`ModalClose`, `kennel_screen.gd:1245`) is a **white "x" (font 16) on a `Color(0,0,0,0.20)` circle, `CLOSE_SIZE = 36` u**. Anchored top-right of the nameplate band, it overlaps the **light** portrait area, so the pale-grey circle + white glyph nearly vanish there — on Sol's and Bella's light-coat portraits the ✕ is barely findable (`.screenshots/P63-close-sol2.png`, 9× zoom: a faint grey disc with an almost-invisible white x at the light↔coat boundary). The grid screen's **own** back/close button (`CloseButton`, `kennel_screen.gd:355`) instead uses a **dark `C_INK` glyph on a light `C_CLOSE_BG` circle** — so the two dismiss controls in the *same* screen are styled oppositely. Both are `CLOSE_SIZE = 36` u, i.e. ~19–20 CSS px on the 390-wide phone (base viewport 720×1280, `project.godot`), **below the 44 u the app uses for its own nav pills** (`TRICKS_BTN_HEIGHT`).
- **Why it's wrong:** the ✕ is the modal's visible dismiss affordance (the full backdrop also closes it, but discoverability of the explicit control still matters). A white-on-translucent-black glyph is a **dark-background treatment** applied over a light surface, so it fails exactly where most dogs' portraits are light — the affordance disappears on the very cells it sits on. It's also a **cross-surface inconsistency**: the kennel has two ✕ controls rendered with opposite ink/fill, and neither reaches the app's own touch-target standard — the polish bar wants one clearly-visible, adequately-sized dismiss component reused everywhere.
- **What "good" looks like:** unify both kennel ✕ controls onto **one shared close-button component** — a solid, defined circle (e.g. a light/paper disc with a dark `C_INK` glyph, or a consistently opaque scrim disc) that reads on **any** band/portrait, sized to the app's **44 u** control standard (or a comparable ~44 CSS-px hit area). Keep it top-corner and unobtrusive, but make the glyph and disc clearly present so the ✕ never washes out over a light coat and both kennel screens match.

**Pass-62 directive RESOLVED (kennel modal stat-meter empty segment ~1.22:1 → 4/5 read as 5/5) — re-verified in pixels, pruned.** Task 188 (`c98f623`) repointed the empty pip `C_PIP_EMPTY` #dfe5ea→#b3bcc4. In my own pixels the empty segment now reads as a clearly-present slate track: Nova's **Mot 4/5** shows a distinct grey 5th box (`.screenshots/P63-statmeter-zoom.png`), and Sol's **Energi 4/5 / Fokus 3/5** read their missing 1–2 boxes at a glance (`.screenshots/P63-modal-sol.png`). Filled segments stay DS blue; the empty slot no longer disappears into the card. Resolved; not re-filed.

**Surface sweep — otherwise clean (re-verified this pass):**
- **Training page** (`P63-a-training.png`, `P63-topbar.png`): matches the goal art — blue BRA hero CTA, hamburger + «Triks» + «Kennel» nav pills (full «Kennel», no truncation — 185 holds), gold coin pill «5000», «Sitt» learned bar with opaque track + `%`, cream Labrador on grass, tan path → blue-roofed cottage, white picket fence, gold + rose coins.
- **Completion menu** (`P63-b-menu.png`): DS paper card — Sitt («Trener nå», active pale-blue wash, dark ink), Ligg/Legg deg («Tilgjengelig»), Gi labb («Låst»); «Raser» Bella·Labrador («Aktiv») / Brun lab («Adopter 30» + coin); ghost «Vis frem hundene»/«Gi tilbakemelding» + solid-blue «Fortsett treningen» primary; BRA correctly dimmed under the scrim. «Raser» subheading AA-clean (186 holds).
- **Breed showcase** (`P63-showcase.png`): «Mine hunder / Bella — aktiv / Labrador» header, spotlit Labrador facing camera in the garden, «Bella» dog-chip, hint «Adopter flere hunder for å bla» over the stage band (169 holds), «Trener nå» / «Tilbake» actions.
- **Kennel** (`P63-c-kennel-grid.png`): 8 cells fill the portrait, front-¾ full-body poses, **Norwegian** rarity/status corner badges (dark ink on calm accents), coin price chips (owned Bella cell has none, Bella's cell keeps its faint-green ownership tint); Nova/Sol/Bella modals open on tap with a hero bust on a calm nameplate band (187 coat parity holds), blurb, 4 blue stat meters (188 fix), «Raseegenskaper» chips (`C_TRAIT_INK` #3a6a9a on #e8f0f8 ≈ 4.9:1, AA-clean by token), «Unikt trekk» card, «Kan lære: Sitt · Ligg · Legg deg», blue «Adopter 🪙 …» CTA. (Only the modal ✕ affordance falls short — see directive above.)

**No regression** in any signed-off phase: every surface renders as before, and closing the kennel/menu restores the Phase-6 training page + Phase-1/2 core mark loop. The directive above is a pre-existing affordance/consistency gap in the kennel's ✕ controls, not something 188 touched.

**No sign-off.** Phases 1/2/3/5/6/8/9 remain signed with no regression. Phase 10 (`phase10.md`) is still **empty/deferred** — owner-gated (no spec ⇒ cannot Visual-Review, cannot invent scope). This pass re-verifies the 188 stat-meter fix, prunes it, and files **one buildable X-6/X-4 directive** (kennel modal ✕ close button under-sized, low-contrast on light portraits, inconsistent with the grid back-✕). The standing asset flags (distinct per-breed **models**, camera-facing **signature clips**, warm human "Bra!"/Maren voice, coat UV re-export) remain owner-gated.
