...
Replayed the full live loop on a 390×844 portrait viewport — driving the real BRA
button through the real scoring path, freezing exact poses to compare "what it looks
like" against "what it scores," and chasing every suspected defect to ground. Phase 1
and the shipped Phase-2 stories all hold up; I found **no open play-test defect** in
the built scope this pass.

What I verified, live:
- **Scoring is honest end to end.** A HOLD tap scores PERFECT/OK/MISS correctly
  through the real button; the window is inclusive at apex±200 ms (OK at the edge),
  the PERFECT band is apex±80 ms, and a dead tap in IDLE scores NONE and does nothing.
- **The apex tell is honest.** The ring is dark (`--apex` 0) in IDLE / empty air and
  ramps linearly to its peak *exactly* on the apex, fading back to dark by the window
  edge — so through the static-looking seated plateau the ring still fades, marking
  the real scoring peak and never lingering past it (P1-4).
- **The dog reads** as a centered, shadow-anchored Labrador on a clean bright
  backdrop; the sit builds as one continuous fold to a clear apex; OK vs PERFECT
  reactions are distinct, grounded (paws on the shadow), and joyful by pose alone.
- **Ligg (P2-1/P2-2)** is selectable from the chip row, swaps at the next idle, and
  plays a distinct, clean lie-down live — stand → play-bow → flat sphinx → stand,
  unmistakably *down*, not a reused sit — and scores PERFECT through the real button.
- **Learned bar (P2-4)** fills on real marks (~12 %/PERFECT, mastery ~9 marks), is
  per-trick (Ligg shows its own fill, not Sitt's), and fires "Mestret!" at 100 %.
- **Persistence (P2-5)** survives a reload (72 % → 72 % after flush + reload); pause
  dims the scene, shows ▶, and a BRA tap while paused does nothing. Zero console
  errors across load, play, and reload.

Pruned this pass (replayed and confirmed fixed): the residual *"looks-seated-but-MISS"
release tail* (was C1). RELEASE now eases *out* — at the window close (apex+200 ms) the
dog is still fully seated and scores OK; 20 ms later it has visibly lifted to ~92 % and
50 ms later to ~80 %, rear clearly off the shadow with the ring already dark. "Looks
fully seated" now ends where "is markable" ends (the old smoothstep held it at 98.5 %
out to apex+250 ms).

*Scope note: audio still can't be heard in the headless harness, so the "Bra!"
voice/SFX is judged only as wired + correctly gated (silent on MISS / dead tap / while
paused), not on how it sounds — worth an on-device listen before final sign-off.*
