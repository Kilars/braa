...
1. **Read the vision.** Skim `.docs/specs.md` so you know what the game is *supposed* to
   be and how it should feel. That is your bar.
2. **Run the real game.** Start the dev server (`bun run dev`; reuse one if already
   running) and drive the actual running app with the Playwright tooling
   (`scripts/shoot.mjs` — supports `--click`, `--wait`, `--eval`, `--poll`, `--report`)
   on a **phone-portrait** viewport (390×844). Take screenshots. Tap the **BRA** marker,
   exercise the timing/scoring, swap marker phrases, watch the dog and its engagement,
   open the economy / kennel / menus — actually *play*.
...
**Append Product Owner notes to `.docs/specs.md`. That is the only file you may touch,
...
- **If the game already matches the spec** and you genuinely find nothing worth changing
  after really playing it, **leave `.docs/specs.md` byte-for-byte unchanged** (add nothing,
