...
// open each light-coat dog modal to check the ✕ over a light portrait
...
for(const id of ["sol","bella","nova"]){
  ...
  await cap("modal-"+id);
  // close via the ✕ to prove it works
  const close = await g("kennel_modal_close");
  if(close){ await tapVp(close.x, close.y); await page.waitForTimeout(500); }
  else { // fallback: reopen grid by tapping backdrop top
    await page.waitForTimeout(300);
  }
  const stillOpen = await g("kennel_modal_open");
  console.log(id, "modal closed after ✕ tap? open=", stillOpen);
  await page.waitForTimeout(300);
}
