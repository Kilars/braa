// PO father-pass-85 play-test drive. Boots the fresh local bundle, drives real taps,
// screenshots every persistent surface, and verifies task 207 (the «Gratis» coral
// price-chip label now clears WCAG AA in-pixel) with a zoomed crop of Trulte's cell.
// Usage: env -u LD_LIBRARY_PATH node tools/po_pass85_drive.mjs <bundle-dir>
...
try {
  ...
  // Zoomed crop around Trulte's cell (secret «Gratis» coral chip) for in-pixel AA check.
  const trulte = cells.find(c => c.id === "trulte");
  if (trulte) {
    const box = await page.locator("canvas").boundingBox();
    const vp = await page.evaluate(() => window.__bra_viewport || [GODOT_W, GODOT_H]);
    const sx = box.width / vp[0], sy = box.height / vp[1];
    const cx = box.x + trulte.x * sx, cy = box.y + trulte.y * sy;
    await shot("03b-trulte-crop", { x: Math.max(0, cx-80), y: Math.max(0, cy-70), width: 160, height: 160 });
  }
