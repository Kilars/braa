...
try {
  ...
  console.log("kennel_btn:", JSON.stringify(kb), "viewport:", JSON.stringify(await g("viewport")));
  await tapVp(kb.x, kb.y);
  await page.waitForTimeout(2500);
  console.log("kennel_open:", await g("kennel_open"));
  await page.screenshot({ path: `${P}after-kennel-tap.png` });
  const cells = await g("kennel_cells");
  console.log("cells:", JSON.stringify(cells));
  const nova = cells.find(c => c.id === "nova");
  await tapVp(nova.x, nova.y);
  await page.waitForTimeout(1600);
  await page.screenshot({ path: `${P}modal-nova.png` });
  ...
  console.log("action2 (after adopt):", JSON.stringify(action));
  await page.screenshot({ path: `${P}modal-owned.png` });
  // train with Nova
  if (action) { await tapVp(action.x, action.y); await page.waitForTimeout(2200); }
  await page.screenshot({ path: `${P}training-nova.png` });
  console.log("kennel_active:", await g("kennel_active"), "active_breed:", await g("active_breed"));
  // open menu
  await tapVp(70, 32);
