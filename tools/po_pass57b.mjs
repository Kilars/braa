...
try {
  ...
  // showcase ("Vis frem hundene")
  const sr = await g("showcase_row");
  console.log("showcase_row", sr);
  if (sr) { await tapVp(sr[0], sr[1]); await page.waitForTimeout(1200); console.log("showcase_open", await g("showcase_open")); await shot("showcase");
    // back out
    const sb = await g("showcase_buttons"); console.log("showcase_buttons", sb);
  }
  code = 0;
