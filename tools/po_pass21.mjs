...
await page.screenshot({ path: ".screenshots/P21-01-training.png" });
...
// completion menu (trick selector / breeds / words)
await tapVp(70, 32);
await page.waitForTimeout(1400);
await page.screenshot({ path: ".screenshots/P21-02-menu.png" });
...
await page.screenshot({ path: ".screenshots/P21-03-kennel-grid.png" });
...
// open a modal on a buyable dog (Nova, 2nd cell)
if (cells && cells[1]) await tapVp(cells[1].x ?? cells[1][0], cells[1].y ?? cells[1][1]);
else if (kb) await tapVp(kb.x, 330);
await page.waitForTimeout(1400);
await page.screenshot({ path: ".screenshots/P21-04-kennel-modal.png" });
