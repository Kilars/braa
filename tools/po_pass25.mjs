// PO father pass 25 — verify 160 (MSAA + 2x portrait SubViewport, clean muzzle) + hunt.
// Serves build/web over http, drives at 390x844 dsf3, captures HUD/menu/kennel/modals.
...
try {
  ...
  console.log("app ready; trick:", await page.evaluate(() => window.__bra_current_trick ?? null),
    ...
    "dog:", await page.evaluate(() => window.__bra_dog_path ?? null));
  ...
  // Sol (idx3) affordable-looking modal for adopt-button state variety
  if (cells && cells[3]) await tapVp(cells[3].x ?? cells[3][0], cells[3].y ?? cells[3][1]);
  await page.waitForTimeout(1600);
  await page.screenshot({ path: `${P}06-modal-sol.png` });
