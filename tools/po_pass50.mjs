...
try {
  ...
  console.log("viewport:", JSON.stringify(await g("viewport")), "trick:", await g("current_trick"), "balance:", await g("balance"));

  // Open completion menu via Triks pill (left of kennel btn)
  ...
  // Open kennel from training (close menu first by tapping kennel btn area on training).
  // Re-load clean, then tap kennel pill directly.
  await page.goto(`${base}?bra_coins=2000`, { waitUntil: "load", timeout: 90000 });
  await page.waitForFunction("window.__appReady === true", undefined, { timeout: 120000 });
  ...
  console.log("kennel_cells:", JSON.stringify(await g("kennel_cells")));
  console.log("kennel_owned:", JSON.stringify(await g("kennel_owned")));

  // Open a kennel modal - tap a non-owned cell (Nova, typically cell index 1)
