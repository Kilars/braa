...
let code = 1;
try {
  // autotap on so marks fire; capture the payoff frames (PERFECT verdict + word-pop)
  ...
  // burst-capture to catch a PERFECT payoff frame with word-pop up
  for (let i = 0; i < 24; i++) {
    await page.waitForTimeout(220);
    await shot(`burst-${String(i).padStart(2,"0")}`);
  }
  code = 0;
