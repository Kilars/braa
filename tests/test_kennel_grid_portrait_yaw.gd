...
## dead-front mugshot (~14°); upper bound rules out a side-profile (~39°). PO target
## was ~15°–38°; these give a hair of margin so the constants aren't brittle to the edge.
const BAND_MIN := 0.24   ## ~13.7° — anything flatter reads as a dead-front mugshot
const BAND_MAX := 0.68   ## ~39.0° — anything wider reads as a broadside side-profile
