...
0. **Scaffolding / core-loop check** — Before the full matrix, answer ONE question:
   - **If there is no runnable app yet** (no `package.json`, or it lacks a `dev` script and the verify gate scripts `typecheck` / `test` / `build` / `e2e`): the **P0 is project scaffolding** — a runnable bun skeleton with those gate scripts and a dev server, so later iterations can build, gate, and (eventually) be play-tested. This overrides all other findings.
