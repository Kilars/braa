...
[`.claude/skills/tdd/SKILL.md`](../tdd/SKILL.md) and the **TDD** section of
`.claude/CLAUDE.md`.
...
- Run `bun run test:verbose` to confirm tests exist and FAIL (red)

Provide a summary of:
- Test file created/modified
- Number of failing tests written and what each covers
- The `bun run test:verbose` output showing red failures (short excerpt only)
```
...
VERIFY WITH COMPACT OUTPUT: run `bun run verify` (typecheck + tests + build).
DO NOT paste full tool logs — report the single summary line (and failing detail
if any). Use `bun run test:verbose` only when debugging a failure.

Provide a summary of:
- Files created/modified
- All acceptance criteria status (checked off in task file)
- The `bun run verify` summary line
...
Fix the issues before marking complete. Re-run `bun run verify`.

### Verification command (compact output)
Use `bun run verify` — a dots wrapper (`scripts/verify.mjs`) that runs
typecheck → tests → build and prints `verify ●●●  ✓ … (N tests)` on success,
dumping a step's full output only when it fails. This keeps subagent/loop
context from filling with vitest per-file lines and the vite build chunk table.
`bun run test:verbose` gives the old per-test output when debugging.
