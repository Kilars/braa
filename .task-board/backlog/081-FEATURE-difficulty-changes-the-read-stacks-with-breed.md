...
- [ ] TDD (RED→GREEN): the per-lever composition is `effective = breed_intrinsic × difficulty.<scale>` for
...
- [ ] In the running game with `?bra_difficulty=hard|expert`, the **read** genuinely changes — tighter
...
- [ ] **X-5 respected:** the tell gets fainter/faster but never collapses to zero information (clamped).
- [ ] **Dormancy proven:** with default Normal, every effective lever equals today's breed-resolved value
