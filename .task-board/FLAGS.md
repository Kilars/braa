...
- **What was tried (adversarial — refute "P3-2 is owner-gated"; this is NOT a premature flag):**
  - The Labrador rig's only clip with the (start, loop, end) structure a mark-timing trick needs **and** a distinct, non-settle action is **`Digging_*`** ("Grav"). It resolved cleanly (`DogClips` dig bundle) and its apex tell/ring fired (2154 gold px). But driven live on the chocolate Lab (`?bra_trick=grav&bra_breed=chocolate`, `tools/web_capture_apex.mjs` + `web_capture_frames.mjs`, 16-frame + 60-frame bursts), the dog plays the dig **rear-to-camera at the scored apex and through the hold** — you read its tail/haunches, not a dog digging (frames `.screenshots/088-grav-choc-07/08/09` + `088-grav-apex-best`, reviewed by eye). That violates the game's PO-enforced **face-the-camera-at-apex** contract (061 P2-11, 077 no-rear-spin, PO note 3 "keep the dog centered/facing" — the PO explicitly rejected the 077 rear-facing celebration as a bug).
  - **A fix was attempted, not just flagged:** a per-trick base-yaw bias of π (`_trick_face_offset`, to let the clip's own ~180° rotation bring the apex back to camera). Re-exported + re-captured (`088-grav-apex-fixed`, frames `f001/f015/f035`): **no effect at the apex** — `_begin_sit` already engages the 061 face-turn generically for every trick, so the base heading was correct; the `Digging` clip drives the body's apex orientation itself (root/hips sweep), which a node-level yaw offset cannot correct. A real fix needs per-frame root-motion extraction + compensation — animation-level surgery, disproportionate + high-risk for one trick, and possibly non-convergent (the dog sweeps through headings mid-dig).
  - The other manifest candidates fail earlier: **`Bark`** (Snakk) is a single clip — no hold-able apex to score; **`Crouch_Idle_*`** reads as a settle-pose, too close to Ligg (not a distinct move); **`Scratching`** is already the ambient feint (071) — reusing it as a headline trick would collide.
- **Decision needed (owner-only):** supply a **camera-facing signature-trick clip** with a hold-able apex (its own start/loop/end, dog facing the player at the apex) — either on a **second real breed model** (P3-D1) or added to the Labrador rig — and lock **P3-D2** (which tricks are signature) / **P3-D4** (per-breed clip coverage). Only the owner can provide the asset + record those decisions.
- **Why it's user-only:** an asset + product-decision gate. Behavior≠inventory was busted honestly here (the manifest WAS checked against live frames, not names) — the clip genuinely exists but is below the readability bar the PO set, and no on-rig clip clears it. This narrows the BUST-068 residual ("breed-exclusive signature-trick clips") from a hypothesis to a **proven** gate.
- **Assumption made to keep going (route 2b — no stub shipped):** task 088 ships **no** signature trick and **no** breed-divergence — both breeds keep the shared, PO-signed **Sitt / Ligg / Legg deg** core (the 088 Grav wiring + per-breed `trick_list` was fully reverted, so the tree carries no rear-to-camera trick and no dead divergence seam). P3-2's "non-identical trick lists" is therefore **deferred to this owner asset**, not silently unmet. The per-breed-trick-list infrastructure is a bundle-add + a label away (065/067 pattern) the moment a usable clip arrives — no architecture is owed.
...
- **Narrowed residual (genuinely owner-gated — stays flagged):** the additional breed **models** (Border Collie / French Bulldog / Husky visual assets) + a breed-exclusive **signature-trick clip** — the rig has no camera-facing action clip with a hold-able apex (`Digging` is present but plays rear-to-camera → proven below-quality by task 088's flag-bust, see the **2026-07-03 P3-2 divergence flag** above); and the three **owner-decisions** P3-D1 / P3-D2 / P3-D4. These wait on the owner supplying models/clips + recording the decisions.
- **Chocolate Labrador recolor — busted 2026-07-02 (BUST-074):** a runtime `albedo_color` multiply (≈`#AA7D51` = `Color(0.668,0.491,0.321)`) on the licensed Labrador's **single coat `StandardMaterial3D`** (the glTF `baseColorFactor` is white → all colour is the baked atlas `dog_licensed_Labrador_Albedo.png`, which `albedo_color` multiplies) yields a convincing chocolate coat at phone scale — a genuine **second breed with NO new owner model**, same rig + same Sitt/Ligg/Legg deg roster. **De-gated → build task 076** (its Visual Review pixel-proves the real render; the bust's tint was a computed simulation). The only residual here is cosmetic: the mouth interior (~0.84% of the atlas) tints brownish-red, so a **re-painted chocolate atlas with correct mouth pigment** is a nicety, **not required to ship**. So the multi-breed dimension is no longer *wholly* owner-gated — chocolate builds now; only genuinely *new-look* breeds (Border Collie / French Bulldog / Husky) still need owner models.
- **Assumption while building:** build the economy (then personality) spine against the single Labrador breed (breed #1), behind the PO-signed Sitt/Ligg/Legg deg roster; coins accrue **additively** so the Phase-2 experience is unregressed. The multi-breed selector/adopt UI wires in once the owner supplies ≥1 more breed model and locks D1/D2 — but a **second breed (chocolate, BUST-074) now exists without the owner** for the roster spine to render.
