...
## Backlog (in priority order — generated by scan-project 2026-06-28 round 2, all Phase-1, non-gated)
These come straight from the PO review's reopened Phase-1 bugfixes. All are
render/material/import defects, **reproducible on the local licensed export** — not gated
on the owner-gated 025 deploy. Build per `start-working` (TDD structural guard + the
binding pixel-verify on a 390×844 Web export).
- *(next scan round — the last two PO-reopened items, both non-gated improvements)*
  IMPROVE: tier readout too low-contrast (P1-7, add outline/drop-shadow); the "positive
  reaction" is a lone Bark and doesn't read as celebration (P1-6, use a happy bounce /
  perk-up like `Jump_Place` instead).
