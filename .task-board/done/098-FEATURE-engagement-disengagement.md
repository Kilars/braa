...
**Status**: DONE (closed 2026-06-21, iteration 23) · **Priority**: Medium
**Created**: 2026-06-19 · **Partially delivered**: 2026-06-19 (iteration 12)

## CLOSED — every slice delivered, none of it needed the imported clips
The old "Depends on 079" gate was **over-broad** (a recurring lesson here — see the board's
orchestration note). Each remaining slice shipped **procedurally on the shipping primitive dog**:
- **Pure model + HUD meter** — task 098 (iteration 12).
- **Reward-latency event wired live** — task 100.
- **Walk-off + call-back on the dog** — task 107.
- **Intermediate itch/flop/bark beats on the dog** — task 112 (this iteration) ← the last slice.

Nothing remains 079-gated. The imported-Labrador clip map (`dogAnimationMap.ts`) additionally
maps every beat to a literal clip for when the licensed model is enabled, but the **procedural
dog expresses the whole §"Wrong-behavior beats & disengagement" arc on its own**. See
tech-decisions §"Engagement Meter + Disengage Beats".

---

_Original on-hold notes (historical) follow._
